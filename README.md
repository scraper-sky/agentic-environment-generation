...
**Live static preview**: https://scraper-sky.github.io/agentic-environment-generation/ (deploys automatically on push via `.github/workflows/deploy-pages.yml`). This is a click-around preview only — it has no backend, so **Generate / Agent / Edit / Rate are disabled there** with an on-screen explanation. **Replay** and **Play** (drive the player yourself with the keyboard) both run entirely client-side and work fully on the hosted preview.

**Live full deploy (Vercel)**: the same app can run with a real backend and the OpenAI key live — `api/*.ts` are Vercel serverless functions mirroring `vite.config.ts`'s dev API, and `src/harness/traverse.ts` exposes a `traverseScene()` entry point that takes an already-parsed scene directly instead of reading one off disk, since a serverless function can't rely on a file written by an earlier request still being there. To deploy: import this repo in Vercel, set `OPENAI_API_KEY` as an environment variable in the project settings, deploy. Two differences from local/CLI use, both because of the serverless environment: the agent is capped at 20 decisions per run instead of 60 (`vercel.json` caps each function at 60s, and each decision is a real OpenAI round-trip), and the feedback/retrieval library (`feedback.json`) is written to `/tmp`, which is ephemeral per invocation — ratings work within a session but aren't guaranteed to persist across cold starts the way they do locally. **This deploy has no rate limiting or access control** — anyone with the URL can trigger real OpenAI calls billed to whatever key is configured; set a hard spending limit on the OpenAI account as the actual safety backstop before sharing the link.
